...
def parse_judgment(raw: str) -> Judgment:
    ...
    return Judgment(str(obj["winner"]),float(obj["score_a"]),float(obj["score_b"]),str(obj.get("rationale","")))
...
class AIFeedbackJudge:
    ...
    """
    def __init__(self,model_id:str,revision=None,device="cpu"):
        from transformers import AutoModelForCausalLM,AutoTokenizer
        self.tokenizer=AutoTokenizer.from_pretrained(model_id,revision=revision)
        if device=="auto":
            self.model=AutoModelForCausalLM.from_pretrained(model_id,revision=revision,device_map="auto",torch_dtype="auto")
        else:
            self.model=AutoModelForCausalLM.from_pretrained(model_id,revision=revision,torch_dtype="auto").to(device)
